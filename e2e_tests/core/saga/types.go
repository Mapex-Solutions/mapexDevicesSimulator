...
//
// A journey is a sequence of Items run against the live simulator. Each Item is
...
// Item is the unit of work executed by Run. Step and Assert satisfy it. The
// runner does not distinguish between them at execution time; the distinction
// matters for rollback (only Step.Compensate is invoked).
type Item interface {
...
// Step represents a mutating action against the live simulator. The runner
// records executed Steps and replays Compensate in reverse order on completion
// (success or failure) so resources created during the journey are removed
// before the next run.
type Step struct {
	// Name is the qualified identifier of the step (e.g.
	// "devices.CreateDevice") logged by the runner.
	Name string
	...
	// Compensate undoes the mutation Do performed. It is optional. Compensate
	// must be idempotent: when Do never ran (because a previous step failed),
	// Compensate is still invoked and must no-op.
...
// Assert represents a read-only verification of system state. Asserts have no
// Compensate because they never mutate the world.
type Assert struct {
	// Name is the qualified identifier of the assertion (e.g.
	// "logs.AssertLogPresent") logged by the runner.
	Name string

	// Check returns nil when the expectation holds. Returning an error halts
	// the journey and triggers rollback of executed Steps.
