...
The walk-throughs reference images under each folder's `images/`. They are
generated by driving the running app with Playwright, using your installed Google
Chrome (`channel: 'chrome'` — no browser binary download).

Prerequisites:
1. The app serving the SPA on `http://localhost:9100` (`cd frontend && npm run dev`).
2. The engine reachable on `http://127.0.0.1:5055`.
3. The Playwright API package available: `cd frontend && npm i -D playwright`.

Then:

```bash
node INTERNALS/capture-screenshots.mjs
# knobs: UI=http://localhost:9100  SIM=http://127.0.0.1:5055  MODE=hash|history
```

The script seeds the four sample devices (and the UDP gateway) through the API so
the pages render real state, then shoots each page into the folders' `images/`.
The capture scripts live in [`../INTERNALS/`](../INTERNALS/) (developer tooling),
not in this folder.
